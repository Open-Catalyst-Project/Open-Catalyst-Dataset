...
class CombinedRandomly():
    ...
    def _random_sites_on_triangle(self, surfsite_pos, k, added_z):
        ...
        """
        A = np.c_[surfsite_pos[:,0], surfsite_pos[:,1], np.ones(surfsite_pos.shape[0])]
        coef,_,_,_ = scipy.linalg.lstsq(A, surfsite_pos[:,2])
        A, B, C = coef
        rand_x = np.random.uniform(low=np.min(surfsite_pos[:,0]), high=np.max(surfsite_pos[:,0]), size=(k,))
        rand_y = np.random.uniform(low=np.min(surfsite_pos[:,1]), high=np.max(surfsite_pos[:,1]), size=(k,))
        rand_z = np.array([A*sitex + B*sitey + C + added_z for sitex, sitey in zip(rand_x, rand_y)])
