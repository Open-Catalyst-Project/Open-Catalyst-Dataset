...
class Slab(Structure):
    ...
    def get_tasker2_slabs(self, tol=0.01, same_species_only=True):
        ...
        for surface_site, shift in [
            ...
            time_start_inner_loop = time.time()
            ...
            for selection in itertools.product(*combinations):
                ...
                props = {k: [] for k in self.site_properties.keys()}
                for site in fixed:
                    for k in site.properties.keys():
                        props[k].append(site.properties[k])
                ...
                for site in tomove:
                    ...
                    for group in selection:
                        ...
                        fcoords.append(site.frac_coords + [0, 0, shift])

                # sort by species to put all similar species together.
                sp_fcoord_wyckoff_eq = sorted(zip(species, fcoords, props['bulk_wyckoff'],
                                       props['bulk_equivalent']), key=lambda x: x[0])
                species = [x[0] for x in sp_fcoord_wyckoff_eq]
                fcoords = [x[1] for x in sp_fcoord_wyckoff_eq]
                props['bulk_wyckoff'] = [x[2] for x in sp_fcoord_wyckoff_eq]
                props['bulk_equivalent'] = [x[3] for x in sp_fcoord_wyckoff_eq]
                slab = Slab(
                    self.lattice,
                    species,
                    fcoords,
                ...
                slabs.append(slab)
                time_elapsed = time.time() - time_start_inner_loop
                if time_elapsed > 300:
                    break
    ...
    def is_symmetric(self, symprec=0.1):
        ...
        symmops = sg.get_space_group_operations()

        if (
            # symmetric if theres inversion symmetry
            sg.is_laue()
            # symmetric if theres translational symmetry from surface 
            # A to surface B in any of the symmetry operations 
            or any(op.translation_vector[2] != 0 for op in symmops)
            # symmetric if theres a mirror parallel to hkl
    ...
    def symmetrically_add_atoms(self, species, points, coords_are_cartesian=False, properties=None):
        """
        ...
        # Get the index of the corresponding site at the bottom and make sure we're not repeating sites
        points2 = self.get_symmetric_sites(points, cartesian=coords_are_cartesian)
        for i, point2 in enumerate(points2):
            if np.all(point2):
                p = {k: properties[k][i] for k in properties.keys()}
                self.append(species[i], points[i], properties=p, 
                            coords_are_cartesian=coords_are_cartesian)
                self.append(species[i], point2, properties=p, 
                            coords_are_cartesian=coords_are_cartesian)


    def symmetrically_remove_atoms(self, indices, tol=0.1):
        """
        ...
        for i1 in indices:
            ...
            for i2 in eq_indices:
                if i2 == i1 or i2 in removal_list:
                    continue
                if slabcopy[i1].frac_coords[2] < slabcopy[i2].frac_coords[2]+tol and \
                slabcopy[i1].frac_coords[2] > slabcopy[i2].frac_coords[2]-tol:
                    continue
        ...
        if len(removal_list) == 2 * len(indices):
            ...
            warnings.warn("Equivalent sites could not be found for removal \
            for all indices. Surface unchanged.")
    ...
    def symmetrically_remove_atoms(self, indices, tol=0.1):
        ...
        for i1 in indices:
            ...
            for i2 in eq_indices:
                # make sure we're not trying to remove a site that's already 
                # considered or remove a site with the same index as the top
                if i2 == i1 or i2 in removal_list:
                    continue
                # check that 
                if slabcopy[i1].frac_coords[2] < slabcopy[i2].frac_coords[2]+tol and \
                slabcopy[i1].frac_coords[2] > slabcopy[i2].frac_coords[2]-tol:
                    continue
                # check if these two sites are in the same partition or 
                # else it will really mess up the symmetry analysis
                if 'supercell' in slabcopy.site_properties.keys():
                    if slabcopy[i1].supercell != slabcopy[i2].supercell:
                        continue
        ...
        if len(removal_list) == 2 * len(indices):
            ...
            warnings.warn("Equivalent sites could not be found \
            for removal for all indices. Surface unchanged.")
...
class SlabGenerator:
    ...
    def get_slab(self, shift=0, tol=0.1, energy=None):
        ...
        if self.primitive:
            ...
            slab = prim
        energy = 0 if energy == None else energy
        ...
        if self.primitive:
            ...
            ouc = self.oriented_unit_cell if slab_l.a != ouc.lattice.a \
            or slab_l.b != ouc.lattice.b else ouc
    ...
    def get_slabs(
        ...
        for g in m.group_structures(slabs):
            ...
            if symmetrize:
                slabs = self.nonstoichiometric_symmetrized_slab(g[0], check_polar=check_polar)
        ...
        return new_slabs# sorted(new_slabs, key=lambda s: s.energy)
    ...
    def nonstoichiometric_symmetrized_slab(self, init_slab, both=True, check_polar=False):
        ...
        Arg:
            ...
            both (bool): Whether to enumerate both terminations
        Returns:
            Slab (structure): A symmetrized Slab object.
        """

        valid_nonpolar = []
        if init_slab.is_symmetric(symprec=self.symprec):
            return [init_slab]
        if check_polar and not init_slab.is_polar():
            valid_nonpolar.append(init_slab.copy())
            
        nonstoich_slabs = []
        # Build an equivalent surface slab for each of the different surfaces
        for top in [True, False]:
            asym = True
            sites = sorted(init_slab, key=lambda site: site.c)
            site_properties = {k: [site.properties[k] for site in sites] 
                               for k in init_slab.site_properties.keys()}
            slab = Slab(init_slab.lattice, [site.species for site in sites],
                        [site.frac_coords for site in sites], init_slab.miller_index,
                        init_slab.oriented_unit_cell, init_slab.shift,
                        init_slab.scale_factor, site_properties=site_properties,
                        reorient_lattice=self.reorient_lattice,
                        energy=0)
            atoms_removed = 0
            ...
            while asym:
                ...
                # Check if the surface is symmetric
                if slab.is_symmetric(symprec=self.symprec):
                    asym = False
                    nonstoich_slabs.append(slab.copy())
                    break
                if not slab.is_polar():
                    valid_nonpolar.append(slab.copy())
                ...
                atoms_removed += 1
                if atoms_removed > len(slab.oriented_unit_cell):
                    break
                                    
            if not both:
                break
        if len(slab) <= len(self.parent) and not asym:
        ...
        if check_polar and not nonstoich_slabs:
            for slab in valid_nonpolar:
...
def get_symmetrically_equivalent_miller_indices(structure, miller_index, return_hkil=True):
    ...
    miller_index = (miller_index[0], miller_index[1], 
                    miller_index[3]) if len(miller_index) == 4 else miller_index
...
def generate_all_slabs(
    ...
    for miller in get_symmetrically_distinct_miller_indices(structure, max_index):
        gen = SlabGenerator(
            ...
            in_unit_planes=in_unit_planes, symprec=symprec
        )
        slabs = gen.get_slabs(
            bonds=bonds,
            tol=tol,
